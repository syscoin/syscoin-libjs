...
export class GeneralService {
    ...
    public encryptwallet(passphrase: string, extraHttpRequestParams?: any): Observable<string> {
        return this.encryptwalletWithHttpInfo(passphrase, extraHttpRequestParams)
            .map((response: Response) => {
                if (response.status === 204) {
                    return undefined;
                } else {
     ...
     * @param passphrase The pass phrase to encrypt the wallet with. It must be at least 1 character, but should be long.
     */
    public encryptwalletWithHttpInfo(passphrase: string, extraHttpRequestParams?: any): Observable<Response> {
        const path = this.basePath + '/encryptwallet';

        let queryParameters = new URLSearchParams();
        let headers = new Headers(this.defaultHeaders.toJSON()); // https://github.com/angular/angular/issues/6845

        // verify required parameter 'passphrase' is not null or undefined
        if (passphrase === null || passphrase === undefined) {
            throw new Error('Required parameter passphrase was null or undefined when calling encryptwallet.');
        }
        if (passphrase !== undefined) {
            queryParameters.set('passphrase', <any>passphrase);
        }
